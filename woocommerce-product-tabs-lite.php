...
if (!class_exists('WoocommerceProductTabsLite')) :

class WoocommerceProductTabsLite {
    ...
    private function product_has_custom_tabs($product) {
        if($this->tab_data === false) {
            $this->tab_data = maybe_unserialize( get_post_meta($product->id, 'product_tabs', true) );
        }
    ...
    public function product_write_panel() {
        ...
        $tab_data = maybe_unserialize( get_post_meta($post->ID, 'product_tabs', true) );
	 ...
	 * identified by the name 'product_tabs'
	 * 
    ...
    public function product_save_data( $post_id, $post ) {
		...
		if(empty($tab_title) && empty($tab_content) && get_post_meta($post_id, 'product_tabs', true)) {
			// clean up if the custom tabs are removed
            delete_post_meta($post_id, 'product_tabs');
            ...
            $tab_data[] = array('title' => $tab_title,
                                'id' => $tab_id,
                                'content' => $tab_content);
            update_post_meta($post_id, 'product_tabs', $tab_data);
		}
    }
    
    /**
     * Runs various functions when the plugin first activates, verifies that
 ...
 * instantiate class
 */
$woocommerce_product_tabs_lite = new WoocommerceProductTabsLite();
 ...
 */
register_activation_hook(__FILE__, array('WoocommerceProductTabsLite', 'on_activation'));
